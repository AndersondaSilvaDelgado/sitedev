...
class Read extends Conn {
    ...
    public function ExeReadMod($Select, $limite = null, $offset = null) {
        $this->Select = $Select;
        $this->limite = $limite;
        $this->offset = $offset;
        $this->Execute();
    ...
    public function LoginAD($Usuario, $Senha) {
        $this->Conn = parent::getConn();
        $this->Read = $this->Conn->prepare("begin ? := login_import('{$Usuario}', '{$Senha}'); end;");
        $this->Read->bindParam(1, $retorno, PDO::PARAM_INT | PDO::PARAM_INPUT_OUTPUT, 12);
        $this->Read->execute();
        $this->Result = $retorno;
    }
